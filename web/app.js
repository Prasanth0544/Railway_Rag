...
function cleanAnswerText(text) {
  // Patterns that indicate leaked follow-up suggestion text
  // These are generated by buildFollowupChips() and sometimes echoed by the LLM
  const LEAK_PATTERN = /(?:(?:Live status of \d+|Route of train \d+|Trains via \w[\w\s]+|Cancellation charges|Luggage rules|Sleeper luggage limit|TTE duties|Tatkal charges|Refund rules|Train info|Train \d+ info|Trains via [A-Z]+)\s*)+$/i;

  const match = text.trimEnd().match(LEAK_PATTERN);
  if (!match) return { clean: text, leaked: [] };

  const leakedBlock = match[0].trim();
  const clean = text.slice(0, text.lastIndexOf(leakedBlock)).trimEnd();

  // Split the concatenated block into individual suggestions
  const leaked = leakedBlock
    .split(/(?=[A-Z][a-z])/)  // split on capital letter boundaries
    .map(s => s.trim())
    .filter(s => s.length > 3);
    ...
    const processLine = (line) => {
      ...
      if (payload.type === 'meta') {
        ...
        if (cardEl) {
          ...
          // Merge: source-based chips + any suggestions the LLM leaked (deduplicated)
          const followupChips = buildFollowupChips(cleanedAnswer, sources);
          const mergedChips = [...new Set([...leakedSuggestions, ...followupChips])].slice(0, 4);
